...
test('convert with utc offset ->> 0', (assert) => {
  const actual = dateInUtc(0, new Date('2018-4-25 18:30')).toLocaleString();
...
test('convert with utc offset ->> 5.5', (assert) => {
  const actual = dateInUtc(5.5, new Date('2018-4-25 18:30')).toLocaleString();
...
test('convert with utc offset ->> -12', (assert) => {
  const actual = dateInUtc(-12, new Date('2018-4-25 18:30')).toLocaleString();
